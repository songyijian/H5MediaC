/*
	H5MediaC 0.4.2  h5多媒体基础操作控件、 支持audio，video
	作者：songyijian 
	发布：2017.3.22
	...
	API
		...
		Attr
			...
			this.state; 		播放状态，非常准确 （播放时为true）
			this.paused; 		暂停状态，非常准确 （暂停时为true） 【是不是觉得有点脑残，为了照顾原生api习惯 多多益善】
* */
!function(){
	function H5MediaC(obj, ajson) {
		...
		this.duration;			//视频总时长
		this.state = false ; 			//播放时为true
		this.paused = true; 			//暂停时为true
	...
	H5MediaC.prototype.init = function() {
		...
		this.setData(this.data.attr,function(){
			...
			_this.state = !_this.el.pause; 			//播放时为true
			_this.paused = _this.el.paused; 		//暂停时为true
